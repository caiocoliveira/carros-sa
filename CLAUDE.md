...
### ROI/Lucro mensal: usar `_score_roi_efetivo`, NÃO `score_roi` puro
Quando `lance_atual > preco_alvo` mas `≤ preco_max` (zona apertada), o operador real entra acima do alvo: capital empatado cresce e o ROI cai. Antes (até 2026-05-03), planilha exibia ROI baseado no alvo nesses lotes, sugerindo retorno ~2× maior do que a entrada real permitiria (caso real: 273% → 122%). Helper canônico em `sheets._score_roi_efetivo(av, lance_atual)` e `sheets._lucro_absoluto_efetivo(av, lance_atual)`. **Sempre que for exibir ROI/Lucro mensal pro operador, passar `lance_atual` — nunca usar `score_roi` puro do DB pra display.** O campo persistido em `AvaliacaoLote.score_roi` continua sendo intrinsic (caso médio no alvo, calibrado por risco/liquidez) — bom pra ranking algorítmico, ruim pra display direto.

### Cuidado com fallback `webmotors_mediana = FIPE × 0.97` somado a `f_km` saturado
Quando Webmotors live não tem dado, `AvaliadorMercado` cai pra `mediana = FIPE × 0.97`. Se o lote ainda tiver `km << km_mediana_mercado`, `f_km` satura em 1.15 e `preco_giro_fipe = 0.97 × 1.15 × FIPE = 1.115 × FIPE` — duas premissas otimistas em série (atacado < FIPE _e_ km muito boa). Não é bug matemático, mas é **dado fraco**: a âncora de revenda não foi validada com mercado real. Audit (`tools/audit.py`) sinaliza quando `preco_giro_fipe > FIPE × 1.10`. Antes de aceitar lance num lote sinalizado, conferir Webmotors live ou ajustar manualmente.

### Não duplicar `_categoria_de_modelo` — usar a canônica de calibracao_giro
A heurística de inferir categoria (HATCH/SEDAN/SUV/PICAPE/UTILITARIO) por substring no nome do modelo vive em `agents/calibracao_giro._categoria_de_modelo` e cobre ~50 modelos (Polo, Pulse, Fastback, Tiggo, etc). Em 2026-05-03 descobrimos que `orquestrador._calcular_frete` mantinha uma cópia reduzida (5 marcas), causando drift: VW Polo virava OUTRO no frete mas HATCH na calibração de giro → frete da categoria errada. **Toda nova heurística por modelo deve importar de `calibracao_giro` ou estendê-la lá.**
