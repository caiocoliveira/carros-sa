...
## Lições acumuladas (auditoria cruzada de colunas)

- **Upsert de lote é caminho quente de perda de dado.** Listagem carrega subset dos campos que o fetch de detalhe enriquece (`preco_referencia_aa`, `fipe_pct_lance_minimo`, `raw_json.detalhe`, `raw_json.loja`). Quando a listagem roda de novo sobre um lote já detalhado, o upsert precisa **preservar** esses campos explicitamente — caso contrário, a segunda ingestão zera o enriquecimento anterior. Regra: em todo upsert, carregue `existente` antes e faça merge campo-a-campo dos dados que a fonte nova não tem. Coberto em `tests/test_orquestrador.py::test_upsert_lote_preserva_dados_de_detalhe_entre_runs`.
- **Racionais econômicos viram invariantes automáticas.** Perguntas do operador ("faz sentido Lance Máximo > FIPE?", "faz sentido preco_giro_fipe muito diferente da FIPE?") **não** são perguntas pontuais — são contratos do sistema. Se a resposta é "geralmente não", vira check em `carros_sa/tools/audit.py` com faixa declarada e teste em `tests/test_audit_columns.py::TestAuditCrossColumn`. Próxima pergunta desse tipo → invariante, não análise ad-hoc.
- **Nomes históricos mentem.** `preco_giro_fipe` hoje é `webmotors_mediana * f_km`, NÃO FIPE. Renomear campo em `models.py` requer coordenação (contratos imutáveis), mas o docstring do `precificador.py` precisa estar em dia ou analista lê e erra. Regra: quando encontrar docstring desatualizada no arquivo crítico, **corrigir no mesmo commit** — não deixar pra depois.
- **Cross-column checks > single-column bounds.** A auditoria antiga só cobria faixas por coluna. Mas bugs sistêmicos (webmotors_mediana errada, margem zerada) só aparecem no cruzamento (Lance Máximo × FIPE, preco_giro_fipe × FIPE, preco_giro_aa × FIPE). Quando adicionar métrica derivada, adicione também o check que compara com a fonte que a originou.
