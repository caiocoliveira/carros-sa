...
### Sessão AA expirando mid-run dispara DD5 reload infinito (DD8 — 2026-06-12)
Cookies AA têm TTL ~1h, cron roda ~2h+. `garantir_autenticado` roda 1× no início. Quando cookie morre mid-run, `page.goto(lote.url)` redireciona pra `/login` → `body.innerText` = ~186 chars (tela de login). DD5 detecta body curto e `page.reload()` — re-fetcha a MESMA tela de login → mesmos 186 chars. Resultado: 78/113 lotes (69%) viraram "⚠ LAUDO NÃO CAPTURADO" em 4 dias de cron quebrado (2026-06-08 a 11). Fix: `_reauth_se_login_redirect(page)` em scraper_autoavaliar.py — quando `page.url` bate em `/login|/entrar|logout|dados-invalidos` (critério canônico via `_url_indica_login_redirect`, espelhado em `sessao_valida`), lê env `AUTOAVALIAR_EMAIL/PASSWORD` e chama `garantir_autenticado` in-place. Caller (`coletar_detalhe` no loop DD5) re-navega pra URL original após retorno True. **Padrão genérico (LESSONS.md/RC12):** garantia de fronteira (auth, rate-limit, idempotência) feita 1× no início NÃO se sustenta em runs ≥ TTL da fronteira. Detectar boundary-broken na camada QUE CONSOME (`coletar_detalhe` observa redirect) e re-estabelecer in-place. Reload tunelado pelo placeholder vazio (DD5/RC11) não basta — re-fetcha o estado quebrado. Quando adicionar nova validação de fronteira (token rotation, session pinning), reusar a função canônica de detecção em AMBOS os lugares (entrada + mid-run) — caso contrário detecções divergem e drift vira bug de longo prazo. Teste guard: `tests/test_coletar_detalhe_acessar.py::test_body_curto_com_login_redirect_dispara_reauth_e_recupera`.
...
**Caso oposto — quando NÃO propagar sufixo:** checks de *data quality / integridade* do audit (ex.: `_check_preco_alvo_gt_preco_max`, `_check_preco_giro_acima_fipe`, `CHECKS["FIPE (R$)"]`) NÃO são família "_em_viavel". Esses sinalizam **pipeline quebrado** (bug do precificador, FIPE stale, mediana inflada) — operador não decide nada com base neles, são alarmes pra debug do sistema. Display já oculta números via "⚠ LAUDO NÃO CAPTURADO" ou via supressão de viáveis quando relevantes. **Regra prática:** se o check sugere "operador deveria conferir o LAUDO antes do lance" → propagar via sufixo. Se sugere "alguém precisa olhar o pipeline" → NÃO propagar, log do cron / audit `--strict` já é o canal certo.
...
### Mensagens operator-facing fossilizam a fonte antiga depois de workstream migrar a origem do sinal (2026-06-13)
Cenário encontrado em revisão diária: `_check_mediana_distante_fipe` em `audit.py` ainda dizia "provável similares poluídos do AA (modelo errado entre similares)" um mês depois do workstream G ter movido a mediana de mercado pra Webmotors live (`anuncio_webmotors` cache, populado pelo cron `carros-sa webmotors-coletar` em 2026-05-12). Quando o aviso disparasse no cron, operador iria investigar avaliador_mercado procurando bug em similares do AA — código que nem é mais chamado pra essa rota. Causa raiz real (amostra pequena com outlier, FIPE defasada, anúncios stale no cache) ficava obscurecida. Teste `TestAuditMedianaDistanteFipe` também assertava `"similares poluídos"` na string — congelando a referência antiga em test guard. Fix: mensagem atualizada pra nomear Webmotors + 3 causas plausíveis na fonte nova (amostra pequena com outlier, prêmio regional, defasagem FIPE no extremo alto; anúncios antigos no cache, sample ruidosa, depreciação real do modelo no extremo baixo) + tests atualizados pra assertions mais resilientes (`"outlier" in v` em vez de `"similares poluídos"`). **Padrão genérico (LESSONS.md/RC13):** quando workstream desliga / move fonte de um sinal, grep o nome da fonte velha em **mensagens de erro de audit, docstrings dos checks, glossário do sheets e tests de string** ANTES de declarar `✅`. Mensagem que sobrevive vira diagnóstico erroneamente apontado em produção. Estruturalmente, preferir mensagens que **não nomeiem a fonte** ("Mediana de mercado R$ X é Y% da FIPE…") ou que nomeiem por **papel** ("amostra de mercado") em vez de produto ("similares do AA") — caso contrário cada migração exige hunt-and-replace propenso a ficar pra trás.

### Test pollution via `asyncio.get_event_loop()` mistura mal com pytest-asyncio AUTO mode em Python 3.12 (2026-06-13)
6 testes de `test_scraper_paginacao.py` rodavam verdes em isolamento mas falhavam com `RuntimeError: There is no current event loop` quando rodavam depois de `test_webmotors_live.py` (que tem testes async). Causa: pytest-asyncio AUTO mode (`asyncio_default_test_loop_scope=function`) cria event loop fresh por teste async **E fecha o loop** ao terminar. O próximo teste síncrono chamando `asyncio.get_event_loop().run_until_complete(coro)` levanta `RuntimeError` em Python 3.12 (em 3.10/3.11 era só warning + auto-criação). Fix: trocar por `asyncio.run(coro)` — cria loop novo a cada chamada, imune ao estado global. **Padrão genérico (LESSONS.md/RC14):** teste síncrono que driva corrotina deve usar `asyncio.run(coro)`, NUNCA `asyncio.get_event_loop().run_until_complete(...)`. O segundo é semanticamente "use o loop atual" — só funciona se alguém estabeleceu um. Em ambientes pytest-asyncio AUTO, esse "alguém" fecha o loop ao terminar. **Antídoto operacional:** SEMPRE rodar `make test` (suite inteira na ordem default) antes de declarar baseline verde, nunca confiar em `-k` ou rodar arquivo isolado — test pollution só aparece na ordem real de execução.
...
### Imports inline dentro de loops "deixados intencionalmente" raramente são intencionais (2026-06-20)
Revisão diária de 15 cenários simulados achou `audit.py::_build_rows` importando `_score_roi_efetivo` e `_km_por_ano` (ambos de `carros_sa.tools.sheets`) **dentro do loop por linha** — 1 import por iteração, mesmo arquivo já tendo `from carros_sa.tools.sheets import HEADER, _lucro_absoluto_efetivo, _lucro_absoluto_no_alvo` no topo. Sem ciclo (sheets não importa audit), nada justificava o lazy import. Provavelmente sobra de evolução incremental (primeiro a função foi adicionada inline; depois subiu pra topo só `_lucro_absoluto_efetivo` mas as outras ficaram). Bonus: `_lucro_absoluto_no_alvo` estava importado no topo e NÃO usado — dead import. **Padrão genérico:** imports inline dentro de funções valem só em 3 casos legítimos — (a) ciclo de import declarado, (b) lazy load de dependência opcional pesada (LLM client, Playwright), (c) try/except no use-site evitando ImportError a load-time pra módulo opcional. Fora disso, consolidar no topo. Antes de declarar revisão `✅`, grep `from carros_sa\..* import` dentro do corpo de funções e ver se há justificativa documentada — caso contrário consolidar. Custo: 5 minutos. Reduz superfície de bugs (import circular silencioso, módulo movido sem atualizar todos os call-sites) + facilita rastrear quem depende de quem. Fix nesta sessão: 2 imports inline movidos pro topo, 1 dead import removido — paridade total mantida no `make test` (629 verde).

### Follow-up "trivial mas fora de escopo" volta a morder — extrai a constante ANTES do próximo ciclo (2026-07-04)
Revisão diária achou 2 follow-ups abertos há semanas que continuavam produzindo divergência silenciosa: (a) DD8-FU1 (2026-06-06) — `_REFORMA_PESADA_PCT_GIRO` como literal `0.30` embutido em `audit._check_reforma_pesada` enquanto `sheets._REFORMA_PESADA_PCT_GIRO` já era constante nomeada — drift silencioso invisível (P5b assimétrico); (b) P5b violado no display do "Top da rodada" (`cli.py::triagem` linha 540 + `scripts/triagem_diaria.py` linha 123): ambos rankeavam por `roi_pct` (score_roi intrinsic × 100) enquanto sheets/audit/`carros-sa top` rankeiam por LUCRO ABSOLUTO efetivo desde workstream II (2026-05-16). Operador via ordem X após triagem → abria a planilha em seguida e via ordem Y — cognitive dissonance. **Padrão genérico (LESSONS.md/RC16):** follow-ups marcados "trivial mas fora de escopo" no PR de origem têm meia-vida real — cada semana que passa é uma sessão a mais lendo código estruturado errado. Ao fechar um workstream, se um follow-up é (a) constante literal duplicada, (b) view não sincronizada com paridade P5b, ou (c) dead field em models.py — RESERVAR 10 min pra fazer no MESMO PR, mesmo que "cirurgia mínima" resiste. Custo do fix é fixo; custo de deixar cresce. Corrigido nesta sessão: (a) `_REFORMA_PESADA_PCT_GIRO` extraído em audit.py + teste guard `test_threshold_reforma_pesada_paridade_sheets_audit` verifica `audit.PCT == sheets.PCT` a runtime; (b) `ResultadoLote.lucro_absoluto` (novo campo, duck-typed via `_lucro_absoluto_efetivo`) preenchido nos 2 return paths de `_pipeline_lote`, ranking + display atualizados nos 2 lugares (cli.py + triagem_diaria.py). 638 verde (+1 teste guard).

**Antídoto operacional pra próxima revisão diária:** grep antes de fechar a sessão:
- `grep -n "0\.30\|_REFORMA_PESADA_PCT_GIRO" sheets.py audit.py` — checa se constantes duplicadas ainda estão sincronizadas.
- `grep -n "sorted.*roi_pct\|sorted.*score_roi" carros_sa/ scripts/` — displays que rankeiam por métrica antiga.
- `grep -n "roi_pct\|lucro_absoluto" carros_sa/ scripts/` — cobertura P5b em TODAS as views que exibem ranking.

### `str(Enum_str_subclass)` devolve `"ClassName.MEMBER"`, NÃO `.value` — coerce via `.value` antes de comparar (2026-06-27)
Revisão de 16 cenários simulados expôs fragilidade silenciosa em 3 lugares (sheets.py + audit.py) que faziam `str(row.get("severidade") or "").lower()` pra comparar com `"estrutural"`. Em produção isso funciona porque `LaudoCache.severidade_geral: str` (orquestrador grava `laudo.severidade_geral.value` — string crua). Mas `SeveridadeAvaria` é declarada como `class SeveridadeAvaria(str, Enum)`. Surpresa: `str(SeveridadeAvaria.ESTRUTURAL)` devolve `"SeveridadeAvaria.ESTRUTURAL"` (default do Enum sobrescreve o str), NÃO `"estrutural"`. Em lowercase vira `"severidadeavaria.estrutural"` — não bate `"estrutural"` e o sufixo `⚠ ESTRUTURAL` / o check `_check_severidade_estrutural_em_viavel` / o cross-field `Reforma R$ 0 com severidade ≥ média` silenciosamente somem. Hoje nenhum caller passa enum direto, mas a fragilidade é latente: qualquer refactor que persista `LaudoEstruturado.severidade_geral` (enum) sem `.value` explode todos os warnings simultaneamente, sem teste pegando nada (testes existentes passam strings literal `"estrutural"`). Fix: helper canônico `_severidade_str(sev)` em `sheets.py` que faz `sev.value if hasattr(sev, "value") else sev` antes do `str(...).lower()`. Audit importa do sheets pra reusar (não duplicar constante). 4 testes guard em sheets + 2 em audit cobrem o caso enum diretamente. **Padrão genérico:** quando comparar valor de `(str, Enum)` mixin via `str(x)`, o resultado é `"ClassName.MEMBER"`, não o `.value`. Tipos do mixin str funcionam em `==` (`SeveridadeAvaria.X == "x"` é True), mas `str(SeveridadeAvaria.X)` quebra. Antes de qualquer `str(row.get("campo_enum")).lower() == "literal"`, **grep o tipo do campo no modelo origem**: se for enum, usar `.value` ou helper coerce. Antídoto operacional: ao adicionar campo enum num model SQLModel `table=True` com tipo `str`, **NUNCA** trocar pro enum direto sem auditar TODOS os `str(...)` usages no codebase. Test guard de baixo custo: adicionar caso "passar enum direto no dict do helper" em TODA função que compara campo enum coerced.

### Docstring de check de audit descreve causa-raiz do disparo — vira diagnóstico obsoleto quando o workstream muda o cálculo (2026-07-11)
Revisão diária de 16 cenários simulados expôs mais uma ocorrência do padrão RC13 em `_check_lance_maximo_acima_fipe` (audit.py:563). A docstring listava as causas plausíveis quando `preco_max > FIPE × 1.05` dispara: "FIPE stale, mediana de similares poluída ou **cap defensivo do precificador (1.20×FIPE)** batendo com **mediana inflada legítima (Webmotors n≥5 sem cap em avaliador)**". PROBLEMA: ambas as referências finais são código MORTO desde 2026-05-08 (refactor FIPE-only) + 2026-05-12 (workstream G LIVE). O cap `_PRECO_GIRO_FIPE_TETO_PCT_FIPE=1.20` foi removido do precificador; o "cap n<5 em avaliador" idem. Se o check disparar em produção, próximo Claude/operador vai grep `1.20` no precificador procurando o cap — código que não existe → 15 min perdidos até descobrir que a docstring está stale. Causa raiz REAL hoje: `_FATOR_MAX` bumpado acima de 1.15 (audit.py:41 já documenta), FIPE persistido stale, ou alguém reintroduziu mediana no `preco_giro`. Fix: docstring reescrita nomeando SÓ as causas viáveis no design atual (FIPE-only) + ref explícita a `_FATOR_MAX` em `carros_sa/ajuste_km.py`. **Padrão genérico (LESSONS.md/RC17 — registrar após merge):** *toda vez que o precificador ou avaliador_mercado for refatorado (remover cap, mudar âncora, trocar fonte), grep `_PRECO_GIRO_FIPE_TETO\|cap defensivo\|cap n<5\|FIPE×1\.20\|FIPE × 1\.20` em `carros_sa/tools/audit.py` `_check_*` docstrings ANTES de fechar o PR — se algum aparecer, é diagnóstico stale*. RC13 estruturalmente: "mensagens que nomeiam a fonte" morrem toda migração. Preferência: nomear por PAPEL (ex.: "âncora do preço-giro") em vez de por FONTE (`FIPE × 1.20`), quando possível.

### Antídoto operacional pra revisão diária "sem bugs verdadeiros" — grep dead-references (2026-07-11)
Revisão de hoje (16 cenários, todos passando, `make test` 643 verde antes+depois) NÃO achou bugs de comportamento. Achou 2 defeitos de MANUTENIBILIDADE que RC13 e o padrão "imports inline" cobrem:
1. Docstring stale (RC17 acima).
2. 3 imports inline em `avaliador_mercado.py::avaliar` (linhas 142, 168, 191) sem ciclo justificando — `obter_anuncios_cacheados`, `calibrar_dias_giro/faixa_de_idade`, `ajustar_dias_giro/bucket_modelo`. Consolidação no topo: -3 linhas de código, 0 mudança de comportamento.

Padrão da revisão diária que amadureceu: quando 16 cenários passam sanity checks (`Lance Máximo > FIPE`? NÃO; `preco_giro_fipe > FIPE × 1.13`? NÃO; coerência ROI×Lucro? OK; sufixos disparando? OK), o valor da sessão não é achar bug funcional — é **matar dead references e docstrings stale** que o próximo Claude/operador vai gastar 15 min investigando. Grep antes de fechar:
- `grep -n "_PRECO_GIRO_FIPE_TETO\|cap defensivo.*1\.20\|FIPE × 1\.20" carros_sa/` — caps removidos que reaparecem em docstrings de check.
- `grep -n "^\s*from carros_sa\." carros_sa/*.py carros_sa/*/*.py | awk -F: '$2 > 60 {print}'` — imports inline sem justificativa (com ciclo NA MESMA função, deixar).
- Rodar `grep "workstream G\|Workstream G" carros_sa/` e revalidar CADA menção — LIVE desde 2026-05-12 mas várias referências ainda dizem "quando ligar" ou "descontinuados os similares AA" fora de contexto histórico.

Não abrir PR só por dead-reference isolada — junta com outra melhoria funcional (fix real, teste novo) senão o custo do PR-review supera o ganho.
