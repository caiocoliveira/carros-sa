...
Quando o usuário pede "revise e corrija autonomamente" sem direcionar, o caminho que voltou a funcionar (3ª vez consecutiva) foi: (1) `make test` baseline → (2) ler precificador + sheets + audit + agentes principais → (3) ESCREVER UM SCRIPT DE SIMULAÇÃO (`/tmp/sim.py`) com **6+ cenários reais** (gold + edge cases f_km no teto/piso + ESTRUTURAL conf baixa + mediana inflada + zona apertada) → (4) imprimir todas as colunas da planilha pra cada cenário e validar identidades algébricas + relação cross-field → (5) só DEPOIS confirmar bugs e refatorar. Cobertura mínima de cenários: gold + saturação de cada bound + caso patológico onde MÚLTIPLOS sinais coexistem (esses revelam P5d). Pular a simulação leva a "fixes" baseados em leitura — frequentemente errados.
