...
- **Follow-ups (não-bloqueantes):**
  - **DD5-FU1 — Métrica `pct_lotes_via_reload_body_vazio`:** instrumentar via log estruturado (`_log.warning` já presente seria suficiente se o cron persistir stderr em artifact). Aplica RC10/RC11 recursivamente — sem métrica, drift do AA fica invisível.
  - **DD5-FU2 — Cap retroativo no DB:** lotes que congelaram via circuit-breaker (II) por body_text vazio histórico podem precisar de reset manual (`scripts/laudo_reset_tentativas.py`) pra terem nova chance com o retry interno. Próximo cron com `reset_circuit_breaker=true` no `workflow_dispatch` cobre o caso.
