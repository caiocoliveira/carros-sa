...
- **2026-04-25 — cap FIPE no preco_giro (fix de sanity):** Code review descobriu que `preco_giro_fipe` no [`precificador.py`](carros_sa/precificador.py) usava `webmotors_mediana × f_km` SEM nenhum cap em FIPE — apesar do nome. Quando f_km>1 (lote com km baixa) ou similares Auto Avaliar com pool contaminado (mediana > FIPE), o `preco_giro` ficava acima do varejo de referência → `preco_max` podia recomendar lance acima da FIPE, incoerente com modelo de negócio (atacado→reforma→varejo, vende próximo da FIPE). Cap absoluto adicionado: `preco_giro_fipe = min(mediana × f_km, FIPE)` e idem para `preco_giro_aa`. Justificativa agora sinaliza `[cap FIPE: ...]` quando aciona. 3 testes novos cobrem (similares contaminados, km baixa, sweep de cenários garantindo `preco_max ≤ FIPE` sempre). Naming `preco_giro_fipe` mantido por compatibilidade de schema (coluna em `avaliacao_lote`); CLAUDE.md ganhou seção de invariantes do modelo de negócio pra futuras revisões pegarem isso.
