...
### W2 — Sanity-check das colunas + recalibração de priors (2026-05-01) ✅
- **Branch:** `claude/sleepy-wright-yrOYd`
- **Motivação:** Usuário pediu novo sanity-check da relação entre as colunas (FIPE × Lance Máximo × Giro × ROI × Lucro/mês) com foco em "isso faz sentido tomar decisão de compra?". Simulação algébrica com 3 lotes-canônicos do ROADMAP (Polo Track, Compass, Fiesta) confirmou que as identidades fechadas continuam corretas, mas expôs 2 problemas operacionais.
- **Bugs encontrados e corrigidos:**
  1. **Priors `_DIAS_GIRO_DEFAULT_POR_FAIXA` (`carros_sa/agents/avaliador_mercado.py:28-44`) eram OTIMISTAS DEMAIS.** Hatch novo=25d, sedan novo=30d, etc — mas o histórico real (94 vendas em `data/historico/uberlandia_arrematado.csv`) tem média 127d hatch, 98d sedan, 79d SUV, 64d picape. Quando a empresa não tinha ≥3 amostras pra (categoria, faixa) — quase sempre — caía no fallback otimista. Resultado real do Polo Track 2024 nessa simulação: ROI anualizado 507%, Lucro/mês R$ 19.986 — fantasia (Polo real foi 227d/ROI ~63%/ano). Recalibrado contra média histórica: hatch novo 90d / médio 125d / velho 150d; sedan 70/95/115; suv 55/80/100; picape 45/65/80; utilitário e outro mantidos conservadores sem dado real. Mesmo Polo Track agora dá ROI 169% e Lucro/mês R$ 6.661 — ordem de magnitude correta.
  2. **Lotes ✗ "Caro demais" exibiam Lucro/mês e ROI anualizado fictícios** ([`carros_sa/tools/sheets.py:352-371`](carros_sa/tools/sheets.py)). Esses cálculos pressupõem que a empresa COMPRA pelo preço-alvo, mas se o lote é inviável (lance atual > preco_max) e a margem aplicada empurra preco_alvo a 0, score_roi infla a 100%+ e gera ROI 300-500% totalmente fictício na planilha. Operador era induzido a pensar "✗ Caro demais" tinha retorno tangível. Fix: lote inviável (mas com laudo analisado) mostra `—` em Lucro/mês e ROI anualizado; Lance Máximo e Reforma seguem visíveis (são fato/limite, não projeção condicional), Tese segue (não depende de comprar).
- **Cobertura:** 3 testes novos — `test_priors_dias_giro_alinhados_com_historico_real` em [`tests/test_avaliador_mercado.py`](tests/test_avaliador_mercado.py) (guard-rail dos pisos por categoria + ordem novo<medio<velho), `test_lote_inviavel_esconde_lucro_mes_e_roi_anualizado` e `test_lote_viavel_mantem_lucro_mes_e_roi_anualizado` em [`tests/test_exportar_sheets.py`](tests/test_exportar_sheets.py). 2 testes existentes ajustados pra novos priors (Fiesta hatch velho 100d → 150d). Suite total: **375 passed, 2 skipped**.
- **Glossário** ([`carros_sa/tools/sheets.py:_write_glossario_sheet`](carros_sa/tools/sheets.py)) atualizado pra documentar que Lucro/mês e ROI anualizado mostram '—' em lotes ✗ Caro demais.
- **Limitações conhecidas:**
  - Priors recalibrados ainda são prior de fallback. Empresa precisa rodar mais arremate pra granularidade `(categoria, faixa)` ter ≥3 amostras (hoje só `hatch_velho`, `sedan_velho`, `suv_medio` superam o piso). Calibração específica continua dominando quando disponível.
  - Validador de "ROI anualizado >1000%" no audit (`carros_sa/tools/audit.py`) continua tolerando até 1000% — esse limite cobre lotes inviáveis que escapem antes do filtro de exibição (não deveria acontecer com este fix, mas defesa em profundidade).
  - O ajuste de pacote (lotes "Caro demais") esconde duas colunas mas mantém preco_max e reforma — operador continua vendo o teto do que faria sentido pagar, sem o ruído de uma rentabilidade fantasma.
