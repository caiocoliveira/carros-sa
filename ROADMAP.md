...
- **Análise por linha (resposta direta):**
  - **`Lance Máximo > FIPE`?** NÃO em produção saudável — máx observado **92.9% FIPE** (Cenário 2 f_km no teto, Corolla km ultra-baixa). Por construção FIPE-only `preco_max ≤ FIPE × 1.15 × 0.95 × 0.90 ≈ 0.98×FIPE`. Audit `_check_lance_maximo_acima_fipe` (1.05×FIPE) mantido como guard de regressão.
  - **`preco_giro_fipe > FIPE`?** SIM até **109.2% FIPE** (Cenários 1, 2 f_km=1.15 saturado). Design FIPE-only embute — carro km-baixa vale mais que mediana. Todos os 16 abaixo do threshold audit **113%** (`_PRECO_GIRO_FIPE_RATIO_MAX`).
  - **Coerência por linha:** todos os 16 passam `capital_ef × ROI_ef ≈ Lucro` (operator mental math). Zona apertada capturada (Cenário 6: intrinsic 54.5% / efetivo 29.6%). Inviáveis viram score_efetivo negativo (display suprime via viavel=False).
  - **Sufixos operacionais:** ⚠ ESTRUTURAL, ⚠ motor, ⚠ reforma pesada, ⚠ margem-alvo inalcançável disparam nos cenários esperados; combinam com ` + ` em Cenário 14 (Peugeot tudo péssimo: `⚠ ESTRUTURAL + motor + reforma pesada + margem-alvo inalcançável`) e Cenário 11 (Uno FIPE baixa: `⚠ reforma pesada + margem-alvo inalcançável`).
- **Achados em revisão preventiva (defeitos de MANUTENIBILIDADE, sem bug funcional):**
  1. **Docstring stale em `_check_lance_maximo_acima_fipe`** (audit.py:563): listava causas plausíveis do disparo como "cap defensivo do precificador (1.20×FIPE)" e "Webmotors n≥5 sem cap em avaliador" — ambas referenciam código MORTO desde 2026-05-08 (refactor FIPE-only) + 2026-05-12 (workstream G LIVE). Se o check disparar em produção, próximo operador/Claude grep `1.20` no precificador procurando o cap — código que não existe → 15 min perdidos. Padrão RC13 (mensagens que nomeiam a fonte antiga fossilizam).
  2. **3 imports inline em `avaliador_mercado.py::avaliar`** (linhas 142, 168, 191) sem ciclo justificando: `obter_anuncios_cacheados` (webmotors_cache não importa avaliador), `calibrar_dias_giro/faixa_de_idade` (calibracao_giro só menciona avaliador em docstring), `ajustar_dias_giro/bucket_modelo` (popularidade nem menciona). Padrão CLAUDE.md 2026-06-20: imports inline em loops raramente são intencionais.
- **Solução cirúrgica:**
  - `audit.py`: docstring de `_check_lance_maximo_acima_fipe` reescrita nomeando SÓ as causas viáveis no design atual (FIPE stale, mediana reintroduzida, `_FATOR_MAX` bumpado acima de 1.15 em `ajuste_km.py`) + ref explícita ao arquivo/constante — se algo mudar futuro, next Claude segue o link direto.
  - `avaliador_mercado.py`: 3 imports consolidados no topo do arquivo. Sem mudança de comportamento (verificado por 643 verde).
- **Cobertura:** sem testes novos (mudanças são docstring + import shuffle — comportamento intocado). `make test` **643/643 verde** (mesma baseline pré-revisão).
- **Impacto:** próximo Claude/operador lendo `_check_lance_maximo_acima_fipe` entende em ≤30s o que investigar quando o alerta disparar (não gasta tempo procurando cap `1.20×FIPE` que não existe). Auditoria de deps do `avaliador_mercado` fica direta no topo do arquivo.
- **Padrões novos registrados em CLAUDE.md:**
  - **RC17 — Docstring de check descreve causa-raiz que fica stale em refactor de fonte**. Ao tocar precificador/avaliador (remover cap, mudar âncora, trocar fonte), grep `_PRECO_GIRO_FIPE_TETO\|cap defensivo\|cap n<5\|FIPE×1\.20` em `audit.py` `_check_*` docstrings ANTES de fechar PR. Preferência estrutural: nomear por PAPEL ("âncora do preço-giro") em vez de por FONTE ("FIPE × 1.20").
  - **Antídoto operacional pra revisão "sem bugs verdadeiros"**: quando 16 cenários passam sanity checks, o valor da sessão está em matar dead references e docstrings stale. Custa 10 min, evita hunt-and-replace do próximo. Não abrir PR isolado só por dead-reference — junta com outra melhoria funcional.
...
### DD11 — Header AA como âncora captura família de páginas-redirect vazias que DD10 não pegava (2026-07-10) ✅
- **Branch:** `claude/amazing-goldberg-v49yv8`
- **Motivação:** Operador (10ª vez): "garantir que todo carro tem laudo baixado, revisado e link na planilha — se não, identificar razão e resolver pra nunca mais acontecer". Diagnóstico via GitHub Actions runs de `triagem.yml`:
  - **Cron 2026-07-06 18:15 UTC (run 28813282081):** step 10 fim `failure` — 138 lotes exportados, **56 incompletos (40%)**: 56 sem PDF + 56 cache<0.6 + 55 URL inválida. Log do scraper mostra dezenas de URLs de vendors `saga/trivel/etc.` devolvendo `DD5 body_text curto (186 chars)` e um caso mostrando `DD8 sessão expirada — re-autenticada — re-navegando` seguido de OUTRO `DD5 body_text curto (186 chars)` (mesma URL) — DD8 executou mas body permaneceu 186 chars.
  - **Cron 2026-07-09 18:02 UTC (run 29039193233):** step 10 `cancelled` no timeout de 4h. Log mostra 60+ URLs de vendors `kuruma/saga/urca/lm/umuaramamotors/autojapan/trivel` com o mesmo padrão de 186 chars em cascata, cada uma consumindo ~20s (initial goto + 2 reloads DD5 + wait). O 4h-timeout matou o run mid-loop.
- **Causa raiz:** DD10 (2026-07-03) adicionou `_RE_VEICULO_VENDIDO_REDIRECT` que captura APENAS a variante `"Este veículo já foi vendido"` da página-redirect vazia do AA (~186 chars). O AA tem MAIS variantes (lote removido pelo anunciante, vendor sem lote ativo, timing race entre listagem cache vs detalhe live) — todas compartilham o header `"Com a Auto Avaliar os bons negócios não param."` mas o CORPO abaixo dele diverge. Sem essas outras variantes serem detectadas, `flags.encerrado=False`, `early_exit=None`, `parse_detalhe` segue, `laudo_pdf_url=None`, cache cai em `_laudo_sem_pdf` (0.55), `_tentativas_extracao +=1` a cada cron. Após 3 dias, circuit-breaker congela → planilha exibe eternamente `"⚠ LAUDO NÃO CAPTURADO"` + audit `--strict` reporta `pdf_ausente + cache_confianca_baixa + url_invalida` → cron marca "failed" e a linha fica de piquete pra sempre. DD8 detectando `page.url` == `/login` recuperava só o subset de sessão-expirada — as outras variantes têm `page.url` mantido na do lote, então DD8 não dispara e o reload DD5 re-fetcha o mesmo conteúdo curto.
- **Solução cirúrgica em `carros_sa/scraping/parsers.py`:**
  - Novo `_RE_AA_REDIRECT_HEADER = re.compile(r"(?i)com\s+a\s+auto\s+avaliar\s+os\s+bons\s+neg[óo]cios\s+n[ãa]o\s+param")` — ancorado no header que TODAS as variantes de redirect compartilham (documentado desde DD10 como o texto marketing do topo da página).
  - Novo helper `_e_pagina_redirect_aa_vazia(body_text)` combina **length < 400 chars** (`_AA_REDIRECT_MAX_LEN`) com match no header. Combinação bulletproof: página real de lote tem 20-50KB de conteúdo (specs, laudo, similares, obs) — nunca passa por length. Se por acaso banner de rodapé de uma página real conter o header, o length falha e o check devolve False.
  - `_detectar_encerrado` chama o helper antes do loop de linha-curta. Retorna True → `flags.encerrado=True` → `_persistir_flags_no_lote` grava `detalhe.encerrado=True` → `flags.early_exit="leilao_encerrado"` → pipeline sai cedo → sheets/audit filtram `not r["encerrado"]`.
- **Cobertura:** 5 testes novos em `tests/test_parsers.py::TestDD11RedirectAaHeader`:
  - `test_body_curto_com_header_aa_sem_frase_vendido_marca_encerrado` — variante genérica sem "já foi vendido" (DD10 não pegava): DD11 marca encerrado.
  - `test_body_curto_com_header_aa_sem_acento_marca_encerrado` — tolerância a acentuação (paridade com DD10).
  - `test_body_longo_com_header_aa_no_banner_nao_marca_encerrado` — página real (>400 chars) com header em banner: encerrado=False (proteção contra falso-positivo).
  - `test_body_curto_sem_header_aa_nao_marca_encerrado` — body curto genérico sem header (fixture de parser exercido com corpo mínimo): encerrado=False.
  - `test_e_pagina_redirect_aa_vazia_helper` — cobertura do helper (boundary 400 chars inclusivo + header sem length + length sem header). **643/643 verde** (638 baseline + 5 novos).
- **Impacto esperado próximo cron (2026-07-10 12:34 UTC = 09:34 BRT):**
  - Lotes com `tentativas < MAX_TENTATIVAS_EXTRACAO (=3)` que devolvem 186 chars de qualquer variante: DD11 marca `encerrado=True` imediato → pipeline early-exit → sem `_laudo_sem_pdf`, sem incrementar tentativas, some do audit e da planilha.
  - Lotes já congelados no circuit-breaker (`tentativas >= 3`) do backlog dos 4-5 crons anteriores: precisam do `reset_circuit_breaker=true` UMA VEZ pra destravar. Ação operacional abaixo.
- **Ações operacionais imediatas pós-merge:**
  - **OP1 — Disparar workflow_dispatch com `reset_circuit_breaker=true`** — destrava os ~56 lotes já congelados nos 4+ dias de cron quebrado. Sem isso, DD11 só afeta lotes NOVOS que entrarem no 186-chars-caminho; os antigos continuam presos até `fim_em < now`. Esta sessão dispara essa ação automaticamente pós-merge (`mcp__github__actions_run_trigger`).
- **Padrão genérico (LESSONS.md/RC17 — registrar após merge):** *"Fix pattern-match único aterrizado sem cobertura da família expõe brecha depois". DD10 pegou UMA variante específica ("Este veículo já foi vendido") do que era claramente uma FAMÍLIA de páginas AA-redirect vazias (o próprio comentário de DD10 já sinalizava "Com a Auto Avaliar os bons negócios não param." como header ancorador). Fix baseado em conteúdo específico da variante deixa as outras irmãs escaparem — retorna como "backlog crescente" 4-5 dias depois. Antídoto operacional:*
  - Quando o fix é pra "uma variante" de página/mensagem/redirect vendor, tentar identificar o **elemento ancorador comum à família** (header, badge, footer, endpoint) e ancorar aí em vez do texto variante — mesmo custo de linhas, cobertura N× maior.
  - Se o ancorador é frágil isolado (banner promo que pode aparecer em página real), combinar com um **gate ortogonal** (length, presença de spec labels, URL pattern) pra bloquear falso-positivo. `_e_pagina_redirect_aa_vazia` faz exatamente isso: header + length<400.
- **Limitações conhecidas:**
  - Se AA mudar o header ("Com a Auto Avaliar…"), DD11 vira NO-OP. Mitigação: DD10 continua cobrindo a variante "vendido" específica; log stderr do cron mostra `DD5 body_text curto` em cascata se muitos lotes escaparem — sinal de regressão a investigar.
  - `_AA_REDIRECT_MAX_LEN=400` é heurístico. Página real de lote tem 20-50KB, então 400 tem 50× de folga — se AA um dia enxugar página real pra <400 chars (improvável dado o volume de specs), reavaliar.
  - Não há métrica `pct_lotes_via_dd11` no audit — sem contagem, regressão silenciosa (AA mudar formato do redirect) fica invisível. Follow-up: contar warns "DD5 body_text curto" que resultam em `encerrado=True` no cron stderr.
- **Follow-ups (não-bloqueantes):**
  - **DD11-FU1 — Métrica `pct_lotes_via_dd11` no audit `--strict`:** mesmo padrão de DD8-FU1 (contar lotes que passaram pelo caminho patológico + foram recuperados). Sem métrica, drift do AA em formato de redirect fica invisível até 5 dias depois no backlog.
  - **DD11-FU2 — DD10-FU1 continua aberto (11 lotes saga/lm com body OK mas extração de laudo_pdf_url falhando):** DD11 não afeta esses (body >200 chars, `_laudo_existe_no_body` retorna True, passadas 5-7 rodam mas devolvem None). Investigação separada pra próxima sessão — capturar HTML pós-click com Playwright local.
  - **DD11-FU3 — DD10-FU2 (3 lotes com URL SA-Laudo válida mas PDF não baixou):** continua aguardando circuit-breaker rodar 2-3 crons pós-DD11 pra ver se resolve sozinho.
...
- **Motivação:** Operador (9ª vez) "garantir que todo carro tem laudo baixado, revisado e link na planilha". Snapshot state/db 2026-07-02 20:20 UTC: 100 lotes auditados / **17 incompletos** (83% completo). Últimos **6 runs consecutivos do cron falharam** — audit `--strict` retornando exit 1 no fim (state persiste OK graças ao `if: always()` no push de state/db, mas o run conta como "failed" no CI).
- **Causa raiz — grupo A (3/17):** Lotes arrematados DURANTE o cron. Quando o operador da revenda dá lance vitorioso mid-scrape, AA redireciona `page.goto(lote.url)` pra tela vazia com body ~186 chars: `"14\nCom a Auto Avaliar os bons negócios não param.\n\nEste veículo já foi vendido mas confira as milhares de outras boas ofertas"`. Sem `page.url` mudar (sem redirect pra `/login`), `_reauth_se_login_redirect` (DD8) tampouco dispara. `_detectar_encerrado()` NÃO pega — a frase "vendido" é >40 chars, badge match falha. Resultado: `flags.encerrado=False` + `early_exit=None` → `_laudo_sem_pdf(0.50)` → planilha exibe "⚠ LAUDO NÃO CAPTURADO" perpetuamente + audit `--strict` reporta `pdf_ausente + cache_confianca_baixa + url_invalida` → cron marca "failed".
- **Solução cirúrgica em `carros_sa/scraping/parsers.py`:**
  - Nova regex `_RE_VEICULO_VENDIDO_REDIRECT = re.compile(r"(?i)este\s+ve[íi]culo\s+j[áa]\s+foi\s+vendido")` — ancorada na frase completa "Este veículo já foi vendido" (com/sem acento), NÃO na palavra isolada. Evita falso-positivo em observação de anunciante ("O carro foi vendido anteriormente sem garantia") — teste `test_parse_detalhe_palavra_vendido_em_observacao_nao_falso_positiva` continua passando.
  - `_detectar_encerrado` chama a nova regex antes do loop de linha-curta. Retorna True → `flags.encerrado=True` → `_persistir_flags_no_lote` grava `detalhe.encerrado=True` → `flags.early_exit="leilao_encerrado"` → pipeline sai cedo → sheets filtra `not r["encerrado"]` e o audit também (linhas 158-159 de `laudo_audit.py`).
- **Cobertura:** 2 testes novos em `tests/test_parsers.py`:
  - `test_parse_detalhe_redirect_veiculo_ja_vendido_marca_encerrado` — body real capturado do snapshot, len<200 (< threshold DD5), valida `encerrado=True + early_exit="leilao_encerrado"`.
  - `test_parse_detalhe_frase_ja_foi_vendido_pega_variacoes_de_acento` — variação sem acento ("veiculo ja foi vendido").
- **Aplicação retroativa no snapshot local:** 3 lotes ativos (23078757, 23086004, 23080246) com body salvo em `raw_json.body_text_sample` batendo a regex nova. Auditor cai de 17 → 14 incompletos localmente. Em produção, próximo cron (10 UTC = 07:00 BRT) reprocessa e marca `detalhe.encerrado=True` via pipeline normal — sem migração manual.
- **Não resolvido nesta sessão:** 14 lotes incompletos restantes:
  - **Grupo B (11 lotes):** URLs de saga/lm — body renderiza completo (`STATUS DO LAUDO / Laudo aprovado com apontamento / Acessar`) mas passadas 5-7 (`_CLICK_ACESSAR_NEAR_LAUDO_JS`) + passada 8 (LLM fallback) devolvem `laudo_pdf_url=None`. Modal do vendor abre em nova aba OU JS handler bloqueia. Precisa Playwright live pra investigar — layout DOM específico dos leiloeiros SAGA/LM.
  - **Grupo C (3 lotes):** URL SA-Laudo válida em `raw_json` (`storage.googleapis.com/sa-laudo/...`) mas PDF não baixou no cron. Testei via `httpx.get` direto agora: retorna 200 OK, 9.8MB. `tentativas_extracao=1` (dentro do circuit-breaker), próximo cron re-tenta. Provavelmente 429 transitório ou timeout.
- **Padrão genérico (LESSONS.md/RC16 — registrar após merge):** *"Redirect vazio disfarçado de página válida" é uma classe de falha diferente de "página não renderizou" (DD5/RC11) e "sessão expirada" (DD8).* Sinais distintivos:
  - DD5: body <200 chars, geralmente HTML shell parcial (SPA lenta). Reload recupera.
  - DD8: body ~186 chars + `page.url` bate `/login`. Re-auth + re-nav recupera.
  - DD10: body ~186 chars + `page.url` NÃO mudou + conteúdo específico do redirect (frase-âncora ex.: "já foi vendido", "não encontrado", "removido pelo anunciante"). Reload/re-auth NÃO recupera — o lote está terminalmente indisponível. Marcar encerrado é a resposta correta.

  Antes de adicionar retry+wait+reauth pra body curto, checar se o body-de-tamanho-similar tem conteúdo específico que indica estado terminal, não transitório. Retry em estado terminal desperdiça 3× ciclos e ainda deixa o lote como "incompleto" no audit.
- **Follow-ups (não-bloqueantes):**
  - **DD10-FU1 — Investigar grupo B (11 lotes saga/lm):** Layout DOM de "Grupo Saga Seminovos" e "LM" tem "Acessar" na área STATUS DO LAUDO mas nossa cascata (5-7 click + 8 LLM) não extrai URL. Rodar `carros-sa` com uma URL saga+Playwright non-headless local + capturar HTML pós-click pra ver se o modal abre em nova aba (target=_blank) ou usa `window.open`. Fix pode ser: (a) escutar `page.on("popup")` durante click, (b) trigger específico por vendor no `_CLICK_ACESSAR_NEAR_LAUDO_JS`.
  - **DD10-FU2 — Grupo C follow-up automático via circuit-breaker:** 3 lotes com URL OK + PDF ausente já entram no `reprocessar_lotes_do_db --somente-laudo-pendente` do próximo cron. Se persistir >3 runs (circuit-breaker limit), operador deve rodar `scripts/laudo_reset_tentativas.py --apply` manualmente. Verificar em 2-3 dias.
...
### DD5 — Retry interno em `coletar_detalhe` para body_text vazio (2026-05-29) ✅
- **Branch:** `claude/amazing-goldberg-URpNh`
- **Motivação:** Operador pediu (6ª vez) "todo carro tem laudo baixado, revisado e link na planilha — se não, identificar razão e resolver pra nunca mais acontecer." DD5 era follow-up aberto desde DD4 (2026-05-15): no snapshot 2026-05-13 sobravam **23 lotes** com `body_text=""` retornado por `coletar_detalhe`. Como `_laudo_existe_no_body("")` é False, as passadas 5-7 (trigger "Acessar" pra kuruma/autus/trivel/autojapan) E a passada 8 (LLM fallback DD4 pra vendors novos) NÃO disparavam mesmo em lote que de fato tem laudo aprovado. Resultado: lote caía no `_laudo_sem_pdf` (confidence 0.55) → após 3 ciclos circuit-breaker (II) congelava → "⚠ LAUDO NÃO CAPTURADO" perpétuo. DD4 cobriu vendor fora do template; DD5 cobre o caso anterior: página nem renderizou pra dar uma chance ao DD4.
- **Causa raiz:** `coletar_detalhe` chamava `page.evaluate("() => document.body.innerText")` uma única vez logo após `goto + wait_for_timeout(1500)`. Em SPA pesada / redirect por sessão expirada / Cloudflare challenge / throttle intermitente / iframe não-carregado, esse evaluate pega o body **antes do conteúdo renderizar** e devolve string vazia. Próximas chamadas a `_laudo_existe_no_body(body_text)` e `parse_detalhe(body_text)` operam em vazio sem nenhum aviso de que a página falhou em entregar — pipeline trata como "lote real sem laudo" e cria cache fraco.
- **Solução cirúrgica:** [`carros_sa/scraping/scraper_autoavaliar.py::coletar_detalhe`](carros_sa/scraping/scraper_autoavaliar.py) — após o evaluate inicial, se `len(body_text) < 200` (página real tem 20-50KB; <200 chars é claramente vazio/erro), dispara até 2 retries com `page.reload(wait_until="networkidle") + wait_for_timeout(3000 → 6000)`. Espelha exatamente o padrão de backoff de `baixar_pdf` (15s/30s/60s pra 429). Caminho feliz (~99% dos lotes) intocado — verifica e segue. Custo: 0s em hits normais; até ~9s + 2 navegações em casos anômalos.
- **Cobertura:** 3 testes guard novos em [`tests/test_coletar_detalhe_acessar.py::TestColetarDetalhe`](tests/test_coletar_detalhe_acessar.py) — body_text vazio + reload recupera (caminho feliz pós-retry); body_text vazio em todas as tentativas retorna ("", None) sem queimar passadas 5-8 desnecessárias; body_text ≥200 chars NÃO dispara reload (caminho hot path inalterado). `FakePage` ganhou `reload()` async + `reload_body_texts` opcional pra simular evolução do body. **604 verde** (eram 601, +3 novos).
- **Impacto esperado:** próximo cron noturno detecta lotes em `body_text=""` via retry interno → 60-80% dos 23 lotes do snapshot devem renderizar corretamente na 2ª/3ª tentativa (SPA pendente é transitório) → `_laudo_existe_no_body` passa → passada 8 LLM dispara em vendors novos → confidence sobe ≥0.6 → audit deixa de flagar → planilha mostra "✓ Viável" + link "Ver laudo". Lotes que sobrarem em vazio crônico (Cloudflare bloqueio persistente) caem corretamente em `_laudo_sem_pdf` com confidence 0.55 e o circuit-breaker congela (esperado — input ruim irrecuperável).
- **Padrão genérico (LESSONS.md/RC11):** Quando uma camada de fetch retorna um *placeholder* "vazio mas válido" (string vazia, lista vazia, dict vazio) e a camada seguinte decide o caminho com base nesse placeholder, persistir o resultado como-está condena o lote a caminho de menor recuperação. Defesa: detectar o caso "fetch respondeu mas conteúdo claramente abaixo do mínimo plausível" (heurística simples, ex. `< 200 chars` numa página de 20KB+) e disparar retry no MESMO endpoint antes da próxima camada consumir. Complementa RC10 (DD4): RC10 era "modelo correto diz nada de útil aqui"; RC11 é "página nem chegou a entregar o conteúdo". Combinados, fecham as duas pontas do "extrator ok, input ruim".
- **Limitações conhecidas:**
  - Em retry persistente (Cloudflare bloqueando IP), os 2 reloads não vão recuperar. Esses lotes continuam caindo em `_laudo_sem_pdf` e o circuit-breaker congela após 3 ciclos — esperado. Operador resolve com rotação de IP ou cookie fresh.
  - `_MIN_BODY_TEXT_BYTES=200` é heurístico. Tela de erro do AA tipicamente tem ~100-150 chars ("Sessão expirada" + nav); página real ≥20KB. Gap de margem é grande, baixíssimo risco de falso positivo. Se algum dia uma página legítima ficar < 200 chars, ajustar.
  - Não há instrumentação contando "lotes que precisaram de reload" — sem isso, regressão silenciosa do AA (mudou layout pra renderizar tudo lazy) ficaria invisível até audit reclamar. Follow-up: campo em `LaudoCache` ou log estruturado no cron.
- **Ações operacionais imediatas (decorrentes do review arquitetural do PR #99):**
  - **OP1 — Operador roda 1× `workflow_dispatch reset_circuit_breaker=true`** após este merge. Lotes em `tentativas_extracao >= 3` são filtrados em `reprocessar_lotes_do_db.py:97` ANTES do pipeline rodar — DD5 não os alcança sem o reset. DD6 já implementou a checkbox; só falta usar.
- **Follow-ups (não-bloqueantes, registrados no review do PR #99):**
  - **DD5-FU1 — Métrica `pct_lotes_via_reload_body_vazio`:** com `_log.warning` agora presente, próximo cron deveria persistir stderr em artifact (`.github/workflows/triagem.yml`). Sem isso, drift do AA (mudança que leva 30% dos lotes a renderizar lazy) fica invisível.
  - **DD5-FU3 — Teste guard `test_reload_que_levanta_excecao_nao_quebra_fluxo`:** entregue neste PR após review. Cobre o branch `except Exception as exc: _log.warning(...)` no reload — sem teste, qualquer refactor que troque `except: log` por `raise` quebra silenciosamente.
  - **DD5-FU4 — Validar empiricamente que página de "sessão expirada" do AA tem ≥200 chars:** threshold calibrado por intuição. Se primeiro cron pós-merge logar `body_text curto` em volume + lotes não destravarem, a página de erro pode estar exatamente em ~200 e o gate vira falso negativo. Mitigação: amostrar 1-2 bodies reais via artifact, ajustar threshold pra 100 + regex anti-falso-positivo na palavra "Sessão expirada".
  - **DD5-FU6 — `wait_until="domcontentloaded"` em vez de `"networkidle"` no reload:** se DD5-FU1 mostrar retries demorando ≥25s, Cloudflare challenge tende a manter conexões abertas e nunca atingir networkidle. Padrão já usado em `login` (linha 265) por mesmo motivo.
  - **DD5-FU7 — Estender heurística RC11 às passadas 2-7 (modal lazy):** o DD5 atual só cobre o goto inicial. Modal aberto via `_ABRIR_MODAL_LAUDO_JS` ou `_CLICK_ACESSAR_NEAR_LAUDO_JS` pode renderizar lentamente — se `_extrair_valido()` retorna None mas modal está visível, vale retry da extração com wait maior antes de marcar a passada como falha.
