...
- **Limitações conhecidas / follow-ups:**
  ...
  - **Calibração da confidence devolvida pelo LLM:** prompt instrui o modelo a usar 0.6+ pra "Nada Consta sem lista" e 0.8+ pra "lista explícita de peças". Em produção pode precisar ajuste — monitorar via `_filtrar_laudo_pendente` continuar pegando lotes da camada 4 (sinal de que o LLM está sub-confiante). Calibrar com mostra ≥ 20 lotes pós-merge.
  - **Custo Gemini free tier:** se camada 4 disparar em >50% dos lotes (sinal de falha geral do visual), o free tier pode estourar quota. Em produção atual seria ~22 chamadas/dia — bem dentro do limite. Monitorar.
