...
# (3) retry automático de laudos pendentes → (4) auditoria final de completude.
#
...
#
# (4) auditar_laudos --strict: passo de verificação. Roda LOGO APÓS o retry
# (URLs assinadas ainda dentro da validade ~1h), confirma que cada lote ativo
# tem PDF + LaudoCache.confidence>=0.6 + URL clicável e não expirada. Exit
# code não-zero quando há gap → aparece como ERRO no log do cron + cron mail
# do operador. Sem essa etapa, lotes incompletos vazavam pra planilha sem
# ninguém perceber até alguém clicar manualmente.
...
echo "  Comando: triagem_diaria.py + limpar_decoys_laudo.py + retry pendentes + auditar_laudos --strict"
echo "  Log:     $LOG"
echo ""
