...
VERIFICA_SCRIPT="$REPO_DIR/scripts/verificar_laudos.py"
LOG="/tmp/carros_sa_triagem.log"
CRON_MARK="carros-sa-triagem"
# Pipeline diário: (1) triagem completa → (2) limpeza de decoys de laudo →
# (3) retry automático de laudos pendentes → (4) verificação final de completude.
#
...
#
# (4) verificar_laudos: relatório final pro log de quantos lotes ativos ainda
# estão sem laudo completo (baixado + analisado + link). Faz auto-heal adicional
# (PDFs locais podres, LaudoCache stale que escapou do retry). Exit !=0 sinaliza
# que sobrou algo — o próximo ciclo (ou passada manual) deve resolver.
CRON_LINE="0 7,13 * * * cd \"$REPO_DIR\" && PYTHONPATH=. \"$PYTHON\" \"$SCRIPT\" --empresa carros_uberlandia >> \"$LOG\" 2>&1; PYTHONPATH=. \"$PYTHON\" \"$DECOY_SCRIPT\" >> \"$LOG\" 2>&1; PYTHONPATH=. \"$PYTHON\" \"$RETRY_SCRIPT\" --empresa carros_uberlandia --somente-ativos --somente-laudo-pendente >> \"$LOG\" 2>&1; PYTHONPATH=. \"$PYTHON\" \"$VERIFICA_SCRIPT\" --empresa carros_uberlandia >> \"$LOG\" 2>&1 # $CRON_MARK"
...
echo "  Comando: triagem_diaria.py + limpar_decoys_laudo.py + retry + verificar_laudos.py"
echo "  Log:     $LOG"
echo ""
