...
def test_parse_laudo_textual_extrai_identificadores(pdf_fiesta_real):
    txt = parse_laudo_textual(pdf_fiesta_real)
...
def test_parse_laudo_textual_extrai_status_juridico(pdf_fiesta_real):
    txt = parse_laudo_textual(pdf_fiesta_real)
...
def test_parse_laudo_textual_odometro_flag(pdf_fiesta_real):
    txt = parse_laudo_textual(pdf_fiesta_real)
    # Laudo do Fiesta: "Visualização Impossibilitada" do odômetro → flag falsa
    assert txt.odometro_legivel is False


def test_hash_pdf_estavel(pdf_fiesta_real):
    h1 = hash_pdf(pdf_fiesta_real)
    h2 = hash_pdf(pdf_fiesta_real)
    assert h1 == h2
